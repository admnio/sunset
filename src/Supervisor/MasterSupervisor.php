...
class MasterSupervisor implements Pausable, Restartable, Terminable
{
    ...
    /**
     * The static random token used to create a unique name for this master.
     *
     * @var string|null
     */
    ...
    public function __construct(?string $environment = null)
    {
        ...
        app(SupervisorCommandQueue::class)->flush($this->commandQueue());
    }
    ...
    /**
     * Reset the static token (useful in tests).
     *
     * @return void
     */
    ...
    public function terminate($status = 0): void
    {
        ...
        if (config('sunset.fast_termination')) {
            app(CacheFactory::class)->forget('sunset:terminate:wait');
        }
    ...
    {
        foreach (app(SupervisorCommandQueue::class)->pending($this->commandQueue()) as $command) {
