...
class Supervisor implements Pausable, Restartable, Terminable
{
     ...
     *
     * @var \Admnio\Sunset\Supervisor\SupervisorOptions
     */
    public $options;
    ...
    public function __construct(SupervisorOptions $options)
    {
        ...
        app(SupervisorCommandQueue::class)->flush($this->name);
    }
     ...
     *
     * @param  \Admnio\Sunset\Supervisor\SupervisorOptions  $options
     * @return \Admnio\Sunset\Supervisor\ProcessPool
     */
    ...
    {
        return ! config('sunset.fast_termination') ||
            app(CacheFactory::class)->get('sunset:terminate:wait');
    }
    ...
    {
        foreach (app(SupervisorCommandQueue::class)->pending($this->name) as $command) {
