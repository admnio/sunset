...
class SupervisorProcess extends WorkerProcess
{
    ...
    /**
     * The exit codes on which supervisor should be marked as dead and NOT restarted.
     *
     * @var array
     */
    ...
    protected function reprovision()
    {
        ...
        app(SupervisorCommandQueue::class)->push(
            MasterSupervisor::commandQueue(),
            'Admnio\Sunset\MasterSupervisorCommands\AddSupervisor',
    ...
    public function terminateWithStatus($status)
    {
        app(SupervisorCommandQueue::class)->push(
            $this->options->name,
            'Admnio\Sunset\SupervisorCommands\Terminate',
            ['status' => $status]
        );
    }
